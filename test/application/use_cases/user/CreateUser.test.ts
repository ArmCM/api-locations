...
describe('Use Case: CreateUser', () => {
  test('should resolve with the newly persisted user (augmented with an ID)', async () => {
    // given
    const persistedUser = new UserEntity(null, 'John', 'Doe', 'john.doe@email.com', 'abcd-1234', 'basic', 'male');

    mockUserRepository.getByEmail = jest.fn((): Promise<UserEntity | boolean> => {
      return Promise.resolve(false);
    });
    mockUserRepository.create = jest.fn((): Promise<UserEntity> => {
  ...
  test('should resolve user exist', async () => {
    // given
    const persistedUser = new UserEntity(1, 'John', 'Doe', 'john.doe@email.com', 'abcd-1234', 'basic', 'male');
